...
def generate_launch_description():
    ...
    robots = [
        {'name': 'tb1', 'x_pose': '-3.5', 'y_pose': '5.0', 'z_pose': 0.01},
        {'name': 'tb2', 'x_pose': '-6.0', 'y_pose': '6.0', 'z_pose': 0.01},
    ...
    inception_p0 = get_package_share_directory('inception_p0')
    ...
    urdf = os.path.join(
        inception_p0, 'urdf', 'turtlebot3_' + TURTLEBOT3_MODEL + '.urdf'
    )

    world = os.path.join(
        get_package_share_directory('inception_p0'),
    ...
    map_server=Node(package='nav2_map_server',
        ...
        name='map_server',
        output='screen',
        parameters=[{'yaml_filename': os.path.join(get_package_share_directory('inception_p0'), 'map', 'aws_bookstore.yaml'),
                     },],
    ...
    for robot in robots:
        ...
        spawn_turtlebot3_burger = Node(
            ...
            arguments=[
                '-file', os.path.join(inception_p0,'models', 'turtlebot3_' + TURTLEBOT3_MODEL, 'model.sdf'),
                '-entity', robot['name'],
