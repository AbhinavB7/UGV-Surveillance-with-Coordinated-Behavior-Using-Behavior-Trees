...
const std::string bt_xml_dir =
    ament_index_cpp::get_package_share_directory("inception_p0") + "/bt_xml";

AutonomyNode::AutonomyNode(const std::string &nodeName) : Node(nodeName)
{
  this->declare_parameter("location_file","none");
  this->declare_parameter("person_file","none");
  this->declare_parameter("robot_name","none");
...
void AutonomyNode::create_behavior_tree()
{
  ...
  BT::NodeBuilder builder2 =
      [=](const std::string &name, const BT::NodeConfiguration &config)
  {
    return std::make_unique<CheckForPerson>(name, config, shared_from_this());
  };

  factory.registerBuilder<CheckForPerson>("CheckForPerson", builder2);
